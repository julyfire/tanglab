...
<?php startMainPane(); ?>
			    <!--<header>
			      <h2><a href="#">The Latest 10 Protocols</a></h2>
					...
					<?php
							if($_GET['subject']){
								$sql="SELECT * FROM protocol WHERE subject='".$_GET['subject']."'";
							}
							elseif($_GET['author']) {
								$sql="SELECT * FROM protocol WHERE created_by='".$_GET['author']."'";
							}
							elseif($_GET['date']) {
								$sql="SELECT * FROM protocol WHERE (SELECT CONCAT(YEAR(created_date),'-',MONTH(created_date)))='".$_GET['date']."'";
							}
							else $sql='';
							if(!$errorInfo && $sql) {
								$result=$db->query($sql);
