...
$content="";
$lasttime="";
$lastauthor=$_SESSION['user'];

//time
$date=getdate();
$lasttime=$date{'year'}."-".$date{'mon'}."-".$date{'mday'}." ".$date{'hours'}.":".$date{'minutes'}.":".$date{'seconds'};
//filename
$time2=$date{'year'}.$date{'mon'}.$date{'mday'}.$date{'hours'}.$date{'minutes'}.$date{'seconds'};
$filename="./protocols/".$time2.".php";
//content
$subject="uncategorized";
if($_POST['p_id'] && $_POST['p_id']!='') $id=$_POST['p_id'];
...
$content=str_ireplace("=\"protocols/","=\"",$content); //修改内容中的图片文件路径
...
if(!$errorInfo) { 
	...
	$sql="UPDATE protocol SET title='".$title_db."', subject='".$subject."', modified_date='".$lasttime."', modified_by='".$lastauthor."', content='".$content_db."', abstract='".$brief."' where id=".$id;
	//echo $sql."<br>";
	$result=$db->query($sql);
	$sql="INSERT INTO p_history(p_id,author,date,title,subject,address) VALUES(".$id.",'".$lastauthor."','".$lasttime."','".$title_db."','".$subject."','".$content_db."')";
	//echo $sql."<br>";
	$result=$db->query($sql);
	if($result!=1) {
